...
В отдельных тестах используются собственные mock-объекты для определённого поведения.
...
Запустить файл с помощью фреймворка pytest можно командой `python3 -m pytest bin/test_packet_scan.py`. Результат запуска показан на скриншоте ниже.
